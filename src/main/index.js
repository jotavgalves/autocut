...
function createWindow() {
  const win = new BrowserWindow({
    ...
    webPreferences: { contextIsolation: true, nodeIntegration: false, sandbox: false, preload: path.join(__dirname, "../preload/index.cjs") }
  });
  win.webContents.on("did-fail-load", (_event, errorCode, errorDescription, validatedUrl) => console.error(`Falha ao carregar ${validatedUrl}: ${errorCode} ${errorDescription}`));
...
async function inspectImage(filePath) {
  try {
    ...
    return {
      ...
      dpi,
      widthCm: (metadata.width / dpi) * 2.54,
      heightCm: (metadata.height / dpi) * 2.54,
...
async function openProject() {
  const result = await dialog.showOpenDialog({ title: "Abrir projeto AUTOCUT", properties: ["openFile"], filters: [{ name: "Projeto AUTOCUT", extensions: ["json"] }] });
  if (result.canceled || !result.filePaths.length) return null;
  const filePath = result.filePaths[0];
  return { ok: true, filePath, project: JSON.parse(await fs.readFile(filePath, "utf8")) };
}
...
async function exportJob(job) {
  ...
  if (sourceMeta.width !== job.source.widthPx || sourceMeta.height !== job.source.heightPx) throw new Error("A arte de origem mudou desde que o projeto foi calculado.");

  const dpi = job.source.dpi;
  const margins = marginPixels(job.margin, dpi);
  const limitPx = cmToPx(job.fabric.maxPrintableWidthCm, dpi);
  const results = [];
  const warnings = [];
  if (String(job.output.format).toUpperCase() === "JPEG" && (job.margin.transparent || sourceMeta.hasAlpha)) warnings.push("JPEG não suporta transparência; áreas transparentes foram compostas sobre branco.");
  ...
  for (const slice of job.slices) {
    ...
    const overlay = renderTechnicalOverlay({ width: finalWidthPx, height: finalHeightPx, margins, slice, orientation: job.orientation, dpi, identification: job.identification, baseName: job.baseName, nameSides: job.nameSides });
    if (overlay) pipeline = pipeline.composite([{ input: Buffer.from(overlay) }]);
    pipeline = applyOutputFormat(pipeline.withMetadata({ density: dpi }), job.output);
    await pipeline.toFile(outputPath);

    const savedMeta = await sharp(outputPath, { limitInputPixels: false }).metadata();
    const savedDpi = savedMeta.density || dpi;
    ...
    validation.approved = validation.dimensionsOk && validation.dpiOk && validation.limitOk;
  ...
  const reconstructionOk = reconstructionPx === expectedAxisPx && job.slices.every((slice, i) => i === 0 ? slice.startPx === 0 : job.slices[i - 1].endPx === slice.startPx) && job.slices.at(-1)?.endPx === expectedAxisPx;
...
function marginPixels(margin, dpi) {
  ...
  const top = margin?.top ? cmToCeilPx(size, dpi) : 0;
  const right = margin?.right ? cmToCeilPx(size, dpi) : 0;
  const bottom = margin?.bottom ? cmToCeilPx(size, dpi) : 0;
  const left = margin?.left ? cmToCeilPx(size, dpi) : 0;
  return { top, right, bottom, left, topPx: top, rightPx: right, bottomPx: bottom, leftPx: left };
}
