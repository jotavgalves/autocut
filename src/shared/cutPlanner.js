...
export function planCutJob({
  ...
  const noCut = evaluateNoCut(document, maxPrintableWidthPx, margin, dpi);

  if (noCut.fits) {
    const slice = makeNoCutSlice({ document, margins: noCut.margins, fabric, baseName, namingTemplate, outputFormat, pedido, orientation: noCut.orientation });
    return {
      ...
      maxPrintableWidthPx,
      margins: noCut.margins,
      balanceCuts,
      slices: [slice],
      validation: {
        approved: true,
        errors: [],
        reconstruction: { ok: true, tolerancePx: 0 },
        checks: [
          "A dimensão final no eixo limitado cabe no limite físico do tecido.",
          `Margem exclusiva: ${noCut.margins.placement === "lateral" ? "esquerda/direita" : "superior/inferior"}.`
        ]
      }
    };
  }
...
function evaluateNoCut(document, maxPrintableWidthPx, margin, dpi) {
  const verticalMargins = marginsForOrientation(margin, "vertical", dpi);
  const finalWidthPx = document.widthPx + verticalMargins.axisTotalPx;
  if (finalWidthPx <= maxPrintableWidthPx) {
    return { fits: true, orientation: "vertical", printOrientation: "as-is", rotatedForPrint: false, margins: verticalMargins };
  }

  const horizontalMargins = marginsForOrientation(margin, "horizontal", dpi);
  const finalHeightPx = document.heightPx + horizontalMargins.axisTotalPx;
  if (finalHeightPx <= maxPrintableWidthPx) {
    return { fits: true, orientation: "horizontal", printOrientation: "rotated-90", rotatedForPrint: true, margins: horizontalMargins };
  }

  return { fits: false };
}

function makeNoCutSlice({ document, margins, fabric, baseName, namingTemplate, outputFormat, pedido, orientation }) {
  ...
  const usefulPx = orientation === "vertical" ? document.widthPx : document.heightPx;
  const finalLimitedAxisPx = usefulPx + margins.axisTotalPx;
  return {
    index: 1,
    total: 1,
    startPx: 0,
    endPx: usefulPx,
    usefulPx,
    usefulCm: pxToCm(usefulPx, document.dpi),
    finalLimitedAxisPx,
    finalLimitedAxisCm: pxToCm(finalLimitedAxisPx, document.dpi),
    finalWidthPx,
    finalHeightPx,
    finalWidthCm,
    finalHeightCm,
    seam: labelsForSlice(1, 1, orientation),
...
function buildCandidate(options) {
  ...
  return {
    ...
    margins,
    slices,
    lastSmall: sliceCount > 1 && lastUsefulPx < minimumLastSlicePx,
    score: {
      sliceCount,
      lastBelowDesiredMinimum: sliceCount > 1 && lastUsefulPx < minimumLastSlicePx ? 1 : 0,
...
function buildFileName({ template, baseName, index, total, fabric, finalWidthCm, finalHeightCm, outputFormat, dpi, pedido }) {
  return renderNameTemplate(template, {
    ...
    FORMATO: String(outputFormat || "PNG").toUpperCase(),
    DPI: dpi,
    PEDIDO: pedido || "",
...
function invalidCandidate(orientation, margins, error) {
  return {
    ...
    margins,
    slices: [],
    score: { sliceCount: Number.POSITIVE_INFINITY, lastBelowDesiredMinimum: 1, wastePx: Number.POSITIVE_INFINITY, smallestUsefulPx: 0 },
    validation: { approved: false, errors: [error], reconstruction: { ok: false } }
  };
}
...
function normalizeCuts(cutPositionsPx, totalAxisPx) {
  return [0, ...(cutPositionsPx || []), totalAxisPx]
    .map((value) => Math.round(Number(value)))
    .filter((value) => Number.isFinite(value) && value >= 0 && value <= totalAxisPx)
...
function enforceLinkedValidity(boundaries, totalAxisPx, usefulLimitPx) {
  ...
  for (let i = 1; i < boundaries.length; i += 1) {
    ...
    while (target - cursor > usefulLimitPx) {
...
function buildManualValidation({ boundaries, totalAxisPx, crossAxisPx, dpi, fabric, margin, orientation, minimumLastSliceCm }) {
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    slices.push({
      index: i + 1,
      total,
      startPx: boundaries[i],
      endPx: boundaries[i + 1],
      usefulPx,
      usefulCm: pxToCm(usefulPx, dpi),
      finalLimitedAxisPx: finalAxisPx,
      finalLimitedAxisCm: pxToCm(finalAxisPx, dpi),
      finalWidthPx,
      finalHeightPx,
      finalWidthCm: pxToCm(finalWidthPx, dpi),
      finalHeightCm: pxToCm(finalHeightPx, dpi),
      seam: labelsForSlice(i + 1, total, orientation),
      valid: finalAxisPx <= maxPrintableWidthPx
    });
  }
  ...
  return { approved: slices.every((slice) => slice.valid) && reconstruction.ok, slices, reconstruction, lastSmall, violations, margins };
}
